...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:9999\",  (string) The cosanta server IP and port we're connected to\n"
...
static UniValue getnetworkinfo(const JSONRPCRequest& request)
{
    ...
    if (g_rpc_node->connman) {
        ...
        obj.pushKV("inboundconnections",   (int)g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_IN));
        obj.pushKV("outboundconnections",   (int)g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_OUT));
        obj.pushKV("mnconnections",   (int)g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_VERIFIED));
        obj.pushKV("inboundmnconnections",   (int)g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_VERIFIED_IN));
        obj.pushKV("outboundmnconnections",   (int)g_rpc_node->connman->GetNodeCount(CConnman::CONNECTIONS_VERIFIED_OUT));
