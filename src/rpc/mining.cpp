...
UniValue generateBlocks(const CTxMemPool& mempool, std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {
        auto pblocktemplate(BlockAssembler(mempool, Params()).CreateNewBlock(coinbaseScript->reserveScript, GetWallets()[0]));
...
static UniValue generateblock(const JSONRPCRequest& request)
{
...

    {
        ...
        std::unique_ptr<CBlockTemplate> blocktemplate(BlockAssembler(empty_mempool, chainparams).CreateNewBlock(coinbase_script, GetWallets()[0]));
        if (!blocktemplate) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Couldn't create new block");
        }
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = BlockAssembler(mempool, Params()).CreateNewBlock(scriptDummy, GetWallets()[0]);
        if (!pblocktemplate)
    ...
    if (pblock->IsProofOfStake()) {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Proof-of-Stake is activated!");
    }
    ...
    result.pushKV("coinbasevalue", (int64_t)pblock->CoinBase()->GetValueOut());
