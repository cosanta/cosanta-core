...
static bool SelectBlockFromCandidates(
    ...
    const map<uint256, const CBlockIndex*>& mapCandidateBlocks,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = nullptr;
    ...
    for (auto iter = vSortedByTimestamp.begin(); ; ++iter) {
        ...
        const auto it_candidate = mapCandidateBlocks.find(iter->second);
        if (it_candidate == mapCandidateBlocks.end()) {
            LogError("SelectBlockFromCandidates: failed to find block index for candidate block %s\n", iter->second.ToString().c_str());
            return false;
        }

        const CBlockIndex* pindex = it_candidate->second;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint32_t& nStakeModifier)
{
    ...
    if (pindexPrev->nHeight == 0) {
        ...
        nStakeModifier = static_cast<uint32_t>(0x1234567887654321ULL);
        return true;
    }
    ...
    map<uint256, const CBlockIndex*> mapCandidateBlocks;
    ...
    while (pindex && pindex->GetBlockTime() >= nSelectionIntervalStart) {
        ...
        vSortedByTimestamp.push_back(make_pair(pindex->GetBlockTime(), block_hash));
        mapCandidateBlocks.emplace(block_hash, pindex);
        pindex = pindex->pprev;
    }
    ...
    for (int nRound = 0; nRound < MODIFIER_INTERVAL_SECTIONS_MAX; nRound++) {
        ...
        if (SelectBlockFromCandidates(vSortedByTimestamp, mapCandidateBlocks, mapSelectedBlocks, nSelectionIntervalStop, nStakeModifier, &pindex)) {
...
bool ComputeNextStakeModifierV2(const uint32_t blockTime, const CBlockIndex* pindexPrev, uint32_t& nStakeModifier)
{
    ...
    // The first 64 bits
    const uint256 hashStakeModifier = Hash(ss);
    nStakeModifier = ReadLE64(hashStakeModifier.begin());
...
bool CheckStakeKernelHash(
    ...
    auto max_time = std::min<int64_t>(
                min_time + nHashDrift,
                TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()) + MAX_POS_BLOCK_AHEAD_TIME - MAX_POS_BLOCK_AHEAD_SAFETY_MARGIN);
...
bool CheckProofOfStake(BlockValidationState& state, const CBlockHeader& header, uint256& hashProofOfStake, const Consensus::Params& consensus, const node::BlockManager* blockman, const CChain* active_chain, const CTxMemPool* mempool)
{
    if (header.posBlockSig.empty()) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-pos-sig", "missing PoS signature");
    }

    const auto is_active_block = [&](const CBlockIndex* pindex) {
        return active_chain && pindex && active_chain->Contains(pindex);
    };
    ...
    LOCK(::cs_main);
    if (!txinPrevRef) {
        const CBlockIndex* pindex_header_prev = blockman ? blockman->LookupBlockIndex(header.hashPrevBlock) : nullptr;
        if (is_active_block(pindex_header_prev)) {
    ...
    {
        const CBlockIndex* pindex_lookup = blockman ? blockman->LookupBlockIndex(txinHashBlock) : nullptr;

        if (pindex_lookup && (!active_chain || active_chain->Contains(pindex_lookup))) {
            pindex_tx = pindex_lookup;
        } else {
            const CBlockIndex* pindex_header_prev = blockman ? blockman->LookupBlockIndex(header.hashPrevBlock) : nullptr;
            if (is_active_block(pindex_header_prev)) {
    ...
    {
        pindex_prev = blockman ? blockman->LookupBlockIndex(header.hashPrevBlock) : nullptr;
        ...
        const auto pindex_fork = active_chain ? active_chain->FindFork(pindex_prev) : nullptr;

        if (active_chain) {
            // Just in case, it must never happen.
            if (!pindex_fork) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-fork-point",
                                    "the fork point is not found");
            }

            // Check if UTXO is beyond possible fork point
            if (pindex_fork->nHeight < pindex_tx->nHeight) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-stake-after-fork",
    ...
    // Extract stake public key ID and verify block signature
    {
        ...
        if (!header.CheckBlockSignature(key_id)) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-sig",
