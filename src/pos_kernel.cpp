...
static bool SelectBlockFromCandidates(
    vector<pair<int64_t, const CBlockIndex*> >& vSortedByTimestamp,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = nullptr;
    ...
    for (auto iter = vSortedByTimestamp.begin(); ; ++iter) {
        ...
        const CBlockIndex* pindex = iter->second;
        if (!pindex)
            return error("SelectBlockFromCandidates: failed to find candidate block index");
...
bool CheckProofOfStake(BlockValidationState& state, const CBlockHeader& header, uint256& hashProofOfStake, const Consensus::Params& consensus, const BlockManager* blockman, const CChain* active_chain, const CTxMemPool* mempool)
{
    ...
    const auto lookup_block = [&](const uint256& hash) -> CBlockIndex* {
    ...
    {
        CBlockIndex* pindex_lookup = lookup_block(txinHashBlock);
