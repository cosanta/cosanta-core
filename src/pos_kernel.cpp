...
bool CheckProofOfStake(BlockValidationState& state, const CBlockHeader& header, uint256& hashProofOfStake, const Consensus::Params& consensus, const BlockManager* blockman, const CChain* active_chain, const CTxMemPool* mempool)
{
    if (header.posBlockSig.empty()) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-pos-sig", "missing PoS signature");
    }

    const auto lookup_block = [&](const uint256& hash) -> const CBlockIndex* {
        return blockman ? blockman->LookupBlockIndex(hash) : nullptr;
    };
    ...
    if (!txinPrevRef) {
        if (is_active_block(lookup_block(header.hashPrevBlock))) {
    ...
    {
        const CBlockIndex* pindex_lookup = lookup_block(txinHashBlock);
    ...
    {
        pindex_prev = lookup_block(header.hashPrevBlock);
