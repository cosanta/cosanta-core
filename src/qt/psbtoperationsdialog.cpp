...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(BitcoinUnits::formatWithUnit(BitcoinUnits::COSA, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(BitcoinUnits::formatWithUnit(BitcoinUnits::COSA, *analysis.fee));
...
void PSBTOperationsDialog::showStatus(const QString &msg, StatusLevel level) {
    ...
    switch (level) {
        case StatusLevel::INFO: {
            m_ui->statusBar->setStyleSheet(GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_PRIMARY));
            break;
        }
        case StatusLevel::WARN: {
            m_ui->statusBar->setStyleSheet(GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_WARNING));
            break;
        }
        case StatusLevel::ERR: {
            m_ui->statusBar->setStyleSheet(GUIUtil::getThemedStyleQString(GUIUtil::ThemedStyle::TS_ERROR));
            break;
        }
    }
    m_ui->statusBar->show();
