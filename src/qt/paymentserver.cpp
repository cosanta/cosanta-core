...
const char* BIP71_MIMETYPE_PAYMENT = "application/cosanta-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/cosanta-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/cosanta-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the cosanta: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // cosanta: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click cosanta: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start cosanta: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling cosanta: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom cosanta addressges are not supported
