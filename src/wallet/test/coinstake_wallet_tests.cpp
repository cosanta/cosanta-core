...
BOOST_AUTO_TEST_CASE(coinstake_cannot_be_resent)
{
    ...
    auto txStake = MakeCoinstakeTx(COutPoint(InsecureRand256(), 0), spk, 10 * COIN);
    CWalletTx wtx{txStake, TxStateInactive{}};
    CWalletTx regular{MakeRegularTx(COutPoint(InsecureRand256(), 0), spk, 5 * COIN), TxStateInactive{}};
    ...
    // CanBeResent() checks !IsCoinStake() internally
    {
        ...
        BOOST_CHECK(m_wallet.CanTxBeResent(regular));
    }
}
...
// ============================================================================
// 5. COINSTAKE SUBMISSION
// ============================================================================

BOOST_AUTO_TEST_CASE(coinstake_cannot_be_submitted)
{
    CKey key;
    key.MakeNewKey(true);
    CScript spk = GetScriptForDestination(PKHash(key.GetPubKey()));
    ...
    for (const bool relay : {false, true}) {
        bilingual_str error;
        BOOST_CHECK(!m_wallet.SubmitTxMemoryPoolAndRelay(wtx, error, relay));
...
BOOST_AUTO_TEST_CASE(maturity_uses_correct_constant)
{
    ...
    {
        LOCK(m_wallet.cs_wallet);
        BOOST_CHECK_EQUAL(m_wallet.GetTxBlocksToMaturity(wtx), 101);
    }
}
...
// ============================================================================
// 9. WALLET TX COINSTAKE ACCESS
// ============================================================================

BOOST_AUTO_TEST_CASE(wallet_tx_delegates_is_coinstake)
{
    // CWalletTx should expose the underlying transaction for coinstake checks.
    CKey key;
    key.MakeNewKey(true);
    CScript spk = GetScriptForDestination(PKHash(key.GetPubKey()));
    ...
    BOOST_CHECK(wtx.tx->IsCoinStake());
    BOOST_CHECK_EQUAL(wtx.tx->IsCoinStake(), txStake->IsCoinStake());
}
                                 ...
                                 const CPubKey& kernel_pubkey, const AutocombineParams& params)
{
    ...
    wallet.AutocombineCoinStake(kernel_prevout, kernel_pubkey, params.target_amount,
                                ...
                                res.stakeTx, res.vin_scripts, res.reward,
...
//! A wallet UTXO with a chosen amount. The regtest fixture coinbases are only
//! 0.01 COSA (heights < 11111), so realistic candidates are faked instead:
//! confirmed from the wallet's point of view and, when in_utxo_set is true,
//! visible to FindCoins() through the mempool view.
...
//! The mainnet scenario: 50 COSA rewards sweep into a 950 COSA kernel at
...
//! 950 + 50 >= 2*500 - 1.
BOOST_FIXTURE_TEST_CASE(autocombine_sweeps_small_inputs, TestChain100Setup)
{
...
BOOST_FIXTURE_TEST_CASE(autocombine_respects_stakecombinemax, TestChain100Setup)
{
    auto wallet = CreateSyncedWallet(*m_node.chain, *m_node.coinjoin_loader, *Assert(m_node.chainman), m_args, coinbaseKey);
    const CScript wallet_spk = GetScriptForDestination(PKHash(coinbaseKey.GetPubKey()));
    const CBlockIndex* tip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
...
//! a 100 COSA kernel grows by 50 COSA inputs up to just below
//! 2*stakesplitthreshold - 1.
BOOST_FIXTURE_TEST_CASE(autocombine_old_target_path, TestChain100Setup)
{
    auto wallet = CreateSyncedWallet(*m_node.chain, *m_node.coinjoin_loader, *Assert(m_node.chainman), m_args, coinbaseKey);
    const CScript wallet_spk = GetScriptForDestination(PKHash(coinbaseKey.GetPubKey()));
    const CBlockIndex* tip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
...
BOOST_FIXTURE_TEST_CASE(autocombine_respects_budgets, TestChain100Setup)
{
    auto wallet = CreateSyncedWallet(*m_node.chain, *m_node.coinjoin_loader, *Assert(m_node.chainman), m_args, coinbaseKey);
    const CScript wallet_spk = GetScriptForDestination(PKHash(coinbaseKey.GetPubKey()));
    const CBlockIndex* tip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
...
BOOST_FIXTURE_TEST_CASE(autocombine_skips_bad_candidates, TestChain100Setup)
{
    auto wallet = CreateSyncedWallet(*m_node.chain, *m_node.coinjoin_loader, *Assert(m_node.chainman), m_args, coinbaseKey);
    const CScript wallet_spk = GetScriptForDestination(PKHash(coinbaseKey.GetPubKey()));
    const CBlockIndex* tip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
