        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            for (const CTxOut& txout: tx.vout) {
                for (const auto& spk_man : GetScriptPubKeyMans(txout.scriptPubKey)) {
                    for (auto &dest : spk_man->MarkUnusedAddresses(batch, txout.scriptPubKey, block_time)) {
...
bool CWallet::CanTxBeResent(const CWalletTx& wtx) const
{
    ...
    return
        ...
        !wtx.isAbandoned() &&
        // Don't try to submit coinbase or coinstake transactions. These would fail anyway but would
        // cause log spam.
        !wtx.IsCoinBase() &&
        !wtx.tx->IsCoinStake() &&
...
bool CWallet::SelectStakeCoins(StakeCandidates& setCoins, CAmount nTargetAmount) const
{
    ...
    std::map<COutPoint, Coin> coins;
    {
        ...
        const std::vector<COutput> vCoins{AvailableCoins(*this, &coin_control, std::nullopt, CoinFilterParams{.min_amount = MIN_STAKE_AMOUNT}).All()};
...
bool CWallet::MintableCoins()
{
    if (m_args.IsArgSet("-reservebalance")) {
        ...
        if (!parsed) {
            LogError("MintableCoins() : invalid reserve balance amount\n");
            return false;
        }
