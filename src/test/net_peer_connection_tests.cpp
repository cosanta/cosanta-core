...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    {
        ASSERT_DEBUG_LOG(strprintf("Added connection to 127.0.0.1:%d peer=1", chainparams.GetDefaultPort()));
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG(strprintf("Not opening a connection to localhost, already connected to 127.0.0.1:%d", chainparams.GetDefaultPort()));
