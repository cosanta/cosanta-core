...
BOOST_AUTO_TEST_CASE(calc_cbtx_merkle_root_quorums_skips_coinstake)
{
    ...
    CBlock powBlock = CreateBlock({}, coinbaseKey, m_node.chainman->ActiveChainstate());
...
BOOST_AUTO_TEST_CASE(build_new_list_from_block_skips_coinstake)
{
    ...
    {
        LOCK(cs_main);
        BOOST_CHECK(m_node.dmnman->BuildNewListFromBlock(
            powBlock, m_node.chainman->ActiveChain().Tip(), statePoW,
            m_node.chainman->ActiveChainstate().CoinsTip(), mnListPoW,
            *m_node.llmq_ctx->qsnapman, false));
    }
    ...
    {
        LOCK(cs_main);
        bool result = m_node.dmnman->BuildNewListFromBlock(
            posBlock, m_node.chainman->ActiveChain().Tip(), statePoS,
            m_node.chainman->ActiveChainstate().CoinsTip(), mnListPoS,
            *m_node.llmq_ctx->qsnapman, false);
...
BOOST_AUTO_TEST_CASE(build_new_list_collateral_check_skips_coinstake)
{
    ...
    CBlock posBlock = CreateBlock({}, coinbaseKey, m_node.chainman->ActiveChainstate());
    posBlock.nVersion |= CBlockHeader::POS_BIT;
    ...
    BlockValidationState state;
    {
        LOCK(cs_main);
        bool result = m_node.dmnman->BuildNewListFromBlock(
            posBlock, m_node.chainman->ActiveChain().Tip(), state,
            m_node.chainman->ActiveChainstate().CoinsTip(), mnList,
            *m_node.llmq_ctx->qsnapman, false);
