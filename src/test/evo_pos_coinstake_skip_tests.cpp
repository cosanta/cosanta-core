...
static CScript GetCoinbaseScript(const CKey& key)
{
    return GetScriptForRawPubKey(key.GetPubKey());
}

// =========================================================================
BOOST_FIXTURE_TEST_SUITE(evo_pos_coinstake_skip_tests, TestChain100Setup)
...
BOOST_AUTO_TEST_CASE(calc_cbtx_merkle_root_quorums_skips_coinstake)
{
    ...
    CBlock powBlock = CreateBlock({}, GetCoinbaseScript(coinbaseKey), m_node.chainman->ActiveChainstate());

    // Calculate quorum merkle root for the PoW block — should succeed
    uint256 merkleRootPoW;
    BlockValidationState statePoW;
    {
        LOCK(cs_main);
        BOOST_CHECK(CalcCbTxMerkleRootQuorums(
            powBlock, m_node.chainman->ActiveChain().Tip(),
    ...
    BlockValidationState statePoS;
    {
        LOCK(cs_main);
        bool result = CalcCbTxMerkleRootQuorums(
            posBlock, m_node.chainman->ActiveChain().Tip(),
            *m_node.llmq_ctx->quorum_block_processor,
...
BOOST_AUTO_TEST_CASE(build_new_list_from_block_skips_coinstake)
{
    ...
    {
        LOCK(cs_main);
        BOOST_CHECK(m_node.chainman->ActiveChainstate().ChainHelper().special_tx->BuildNewListFromBlock(
            powBlock, m_node.chainman->ActiveChain().Tip(),
            DeploymentActiveAfter(m_node.chainman->ActiveChain().Tip(), *m_node.chainman, Consensus::DEPLOYMENT_V24),
            m_node.chainman->ActiveChainstate().CoinsTip(), false, statePoW, mnListPoW));
    }
    ...
    {
        LOCK(cs_main);
        bool result = m_node.chainman->ActiveChainstate().ChainHelper().special_tx->BuildNewListFromBlock(
            posBlock, m_node.chainman->ActiveChain().Tip(),
            DeploymentActiveAfter(m_node.chainman->ActiveChain().Tip(), *m_node.chainman, Consensus::DEPLOYMENT_V24),
            m_node.chainman->ActiveChainstate().CoinsTip(), false, statePoS, mnListPoS);
    ...
    BOOST_CHECK(mnListPoW == mnListPoS);
}
...
BOOST_AUTO_TEST_CASE(build_new_list_collateral_check_skips_coinstake)
{
    ...
    CBlock posBlock = CreateBlock({}, GetCoinbaseScript(coinbaseKey), m_node.chainman->ActiveChainstate());
    posBlock.nVersion |= CBlockHeader::POS_BIT;
    ...
    BlockValidationState state;
    {
        ...
        bool result = m_node.chainman->ActiveChainstate().ChainHelper().special_tx->BuildNewListFromBlock(
            posBlock, m_node.chainman->ActiveChain().Tip(),
            DeploymentActiveAfter(m_node.chainman->ActiveChain().Tip(), *m_node.chainman, Consensus::DEPLOYMENT_V24),
            m_node.chainman->ActiveChainstate().CoinsTip(), false, state, mnList);
