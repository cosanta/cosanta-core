...
bool CChainState::InvalidateBlock(CValidationState& state, const CChainParams& chainparams, CBlockIndex *pindex)
{
    ...
    while (it != mapBlockIndex.end()) {
        if (it->second->IsValid(BLOCK_VALID_TRANSACTIONS) && !(it->second->nStatus & BLOCK_CONFLICT_CHAINLOCK) && it->second->HaveTxsDownloaded() && !setBlockIndexCandidates.value_comp()(it->second, chainActive.Tip())) {
...
void CChainState::ResetBlockFailureFlags(CBlockIndex *pindex) {
    ...
    while (it != mapBlockIndex.end()) {
        if (!it->second->IsValid() && it->second->GetAncestor(nHeight) == pindex) {
            ...
            if (it->second->IsValid(BLOCK_VALID_TRANSACTIONS) && !(it->second->nStatus & BLOCK_CONFLICT_CHAINLOCK) && it->second->HaveTxsDownloaded() && setBlockIndexCandidates.value_comp()(chainActive.Tip(), it->second)) {
                setBlockIndexCandidates.insert(it->second);
            }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckProof)
{
...
bool ProcessNewBlockHeaders(std::deque<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid, int burst_limit)
{
    if (first_invalid != nullptr) first_invalid->SetNull();
    {
        ...
        while (!headers.empty()) {
            ...
            if (!g_chainstate.AcceptBlockHeader(headers.front(), state, chainparams, &pindex)) {
                return false;
            }
...
{
    AssertLockHeld(cs_main);
    assert(pindexPrev && chainActive.Contains(pindexPrev));
    ...
    if ((pindexPrev == chainActive.Tip()) &&
            !g_chainstate.ConnectBlock(block, state, &indexDummy, viewNew, chainparams, true))
