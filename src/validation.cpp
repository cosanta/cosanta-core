...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, bool fCheckMasternodesUpgraded, bool isPos)
{
    ...
    int32_t nVersion = g_versionbitscache.ComputeBlockVersion(pindexPrev, params);
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, m_blockman, chainparams, pindexPrev, GetAdjustedTime(), true, &ActiveChainstate().m_chain)) {
...
bool CheckProof(BlockValidationState& state, const CBlockIndex& index, const Consensus::Params& params, const BlockManager* blockman, const CChain* active_chain) {
