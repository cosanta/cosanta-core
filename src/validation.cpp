...
bool GetUTXOCoin(Chainstate& active_chainstate, const COutPoint& outpoint, Coin& coin)
{
    LOCK(cs_main);
...
int GetUTXOHeight(Chainstate& active_chainstate, const COutPoint& outpoint)
{
...
int GetUTXOConfirmations(Chainstate& active_chainstate, const COutPoint& outpoint)
{
    // -1 means UTXO is yet unknown or already spent
    LOCK(cs_main);
...
private:
    ...
    struct Workspace {
        ...
        /** Size of the transaction as used by the mempool, calculated using serialized size
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!ContextualCheckTransaction(tx, state, chainparams.GetConsensus(), m_active_chainstate.m_chain.Tip())) {
        LogError("%s: ContextualCheckTransaction: %s, %s\n", __func__, hash.ToString(), state.ToString());
        return false;
    }
    ...
    // Transactions smaller than 65 bytes are not relayed to mitigate CVE-2017-12842.
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    auto result = m_pool.CheckPackageLimits(txns, m_limits);
    if (!result) {
                                  ...
                                  std::map<const uint256, const MempoolAcceptResult>& results)
{
    ...
    std::vector<uint256> all_package_txids;
    all_package_txids.reserve(workspaces.size());
    std::transform(workspaces.cbegin(), workspaces.cend(), std::back_inserter(all_package_txids),
                   [](const auto& ws) { return ws.m_ptx->GetHash(); });
    // Find the wtxids of the transactions that made it into the mempool. Allow partial submission,
    // but don't report success unless they all made it into the mempool.
    for (Workspace& ws : workspaces) {
        const auto effective_feerate = args.m_package_feerates ? ws.m_package_feerate :
            CFeeRate{ws.m_modified_fees, static_cast<uint32_t>(ws.m_vsize)};
        const auto effective_feerate_txids = args.m_package_feerates ? all_package_txids :
            std::vector<uint256>({ws.m_ptx->GetHash()});
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    const std::vector<uint256> single_txid{ws.m_ptx->GetHash()};
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactions(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    // package feerate (total modified fees / total size) to check this requirement.
...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
    ...
    for (const auto& tx : package) {
        ...
        if (m_pool.exists(txid)) {
            ...
            assert(iter != std::nullopt);
            results_final.emplace(txid, MempoolAcceptResult::MempoolTx(iter.value()->GetTxSize(), iter.value()->GetFee()));
        } else {
    ...
    if (quit_early || txns_package_eval.empty()) {
        for (const auto& [txid, mempoolaccept_res] : individual_results_nonfinal) {
            Assume(results_final.emplace(txid, mempoolaccept_res).second);
...
static SteadyClock::duration time_is_filter{};
static SteadyClock::duration time_subsidy{};
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    time_process_special += time_2_1 - time_2;
    LogPrint(BCLog::BENCHMARK, "      - ProcessSpecialTxsInBlock: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_2_1 - time_2),
    ...
    const auto time_5_1{SteadyClock::now()};
    time_is_filter += time_5_1 - time_4;
    LogPrint(BCLog::BENCHMARK, "      - IS filter: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_5_1 - time_4),
             Ticks<SecondsDouble>(time_is_filter),
             Ticks<MillisecondsDouble>(time_is_filter) / num_blocks_total);
    ...
    const auto time_5_2{SteadyClock::now()};
    time_subsidy += time_5_2 - time_5_1;
    LogPrint(BCLog::BENCHMARK, "      - GetBlockSubsidy: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_5_2 - time_5_1),
    ...
    const auto time_5_3{SteadyClock::now()};
    time_value_valid += time_5_3 - time_5_2;
    LogPrint(BCLog::BENCHMARK, "      - IsBlockValueValid: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_5_3 - time_5_2),
    ...
    const auto time_5_4{SteadyClock::now()};
    time_payee_valid += time_5_4 - time_5_3;
    LogPrint(BCLog::BENCHMARK, "      - IsBlockPayeeValid: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_5_4 - time_5_3),
             Ticks<SecondsDouble>(time_payee_valid),
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    ::g_stats_client->timing("ConnectTip_ms", Ticks<std::chrono::milliseconds>(time_6 - time_1), 1.0f);
...
void Chainstate::ResetBlockFailureFlags(CBlockIndex *pindex, bool ignore_chainlocks) {
    AssertLockHeld(cs_main);
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (const auto& tx : block.vtx) {
        ...
        TxValidationState tx_state;
        if (!ContextualCheckTransaction(*tx, tx_state, chainman.GetConsensus(), pindexPrev)) {
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, const uint256& hash)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, hash, state, GetConsensus())) {
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (!ReadBlockFromDisk(block, pindex, consensus_params)) {
            LogError("Verification error: ReadBlockFromDisk failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pindex->GetUndoPos().IsNull()) {
                if (!UndoReadFromDisk(undo, pindex)) {
                    LogError("Verification error: found bad undo data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
            }
        }
        ...
        if (nCheckLevel >= 3) {
            if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
                ...
                if (res == DISCONNECT_FAILED) {
                    LogError("Verification error: irrecoverable inconsistency in block data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
    ...
    if (pindexFailure) {
        LogError("Verification error: coin database inconsistencies found (last %i blocks, %i good transactions before that)\n", chainstate.m_chain.Height() - pindexFailure->nHeight + 1, nGoodTransactions);
        return VerifyDBResult::CORRUPTED_BLOCK_DB;
    }
