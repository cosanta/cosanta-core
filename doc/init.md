Sample init scripts and service configuration for cosantad
==========================================================
...
Running cosantad as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `cosanta.conf` configuration file to override
...
PID file:            `/var/run/cosantad/cosantad.pid` (OpenRC and Upstart) or `/run/cosantad/cosantad.pid` (systemd)
Lock file:           `/var/lock/subsys/cosantad` (CentOS)

The PID directory (if applicable) and data directory should both be owned by the
cosantacore user and group. It is advised for security reasons to make the
configuration file and data directory only readable by the cosantacore user and
group. Access to cosanta-cli and other cosantad rpc clients can then be
...
cosantacore user and group to do so. This does not allow
for the listing of files under the directory.

NOTE: It is not currently possible to override `datadir` in
`/etc/cosantacore/cosanta.conf` with the current systemd, OpenRC, and Upstart init
files out-of-the-box. This is because the command line options specified in the
init files take precedence over the configurations in
`/etc/cosantacore/cosanta.conf`. However, some init systems have their own
configuration mechanisms that would allow for overriding the command line
options specified in the init files (e.g. setting `COSANTAD_DATADIR` for
OpenRC).
...
setting the COSANTAD_BIN and COSANTAD_OPTS environment variables in the file
/etc/sysconfig/cosantad. You can also use the DAEMONOPTS environment variable here.

### macOS

Copy org.dash.cosantad.plist into ~/Library/LaunchAgents. Load the launch agent by
running `launchctl load ~/Library/LaunchAgents/org.dash.cosantad.plist`.
...
You will need to modify org.dash.cosantad.plist if you intend to use it as a
