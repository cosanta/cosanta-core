...
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Cosanta.
...
When building the dependencies, as described in the [depends README](../depends/README.md), use
...
example /usr/src/cosanta, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
