...
- [Development guidelines](#development-guidelines)
    - [General Cosanta Core](#general-cosanta-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Cosanta Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while cosantad is running with the `logging`
RPC.  For instance, launching cosantad with `-debug` or `-debug=1` will turn on
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run cosantad.

You can then use the `-debug=lock` configuration option at cosantad startup or
`cosanta-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `cosanta-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/cosanta/cosanta-core/blob/master/contrib/valgrind.supp))
...
Cosanta Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`cosantad`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
Some of these are maintained by active developers of Cosanta Core, in which case
...
a Cosanta Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Use *invalid* Cosanta addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
