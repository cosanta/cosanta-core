...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        with open(os.path.join(self.nodes[0].datadir, 'cosanta.conf'), 'a', encoding='utf-8') as conf:
        ...
        self.nodes[0].assert_start_raises_init_error(
            expected_msg='Error: Error parsing command line arguments: Invalid parameter -cosanta_cli=1',
            extra_args=['-cosanta_cli=1'],
        )
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('cosanta_conf=1\n')
        with self.nodes[0].assert_debug_log(expected_msgs=['Ignoring unknown configuration value cosanta_conf']):
            self.start_node(0)
        self.stop_node(0)
        ...
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('-cosanta=1\n')
        self.nodes[0].assert_start_raises_init_error(expected_msg='Error: Error reading configuration file: parse error on line 1: -cosanta=1, options in configuration file must be specified without leading -')
        ...
        main_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'cosanta_main.conf')
    ...
    def test_config_file_log(self):
        ...
        # Write a cosanta.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = pathlib.Path(node.bitcoinconf).read_text()
        conf_path = default_datadir / "cosanta.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a cosanta.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "cosanta.conf" file which is ignored, because a '
        ...
        # the same cosanta.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir}/cosanta.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when cosanta.conf in the default data directory sets another datadir '
                      'and it contains a different cosanta.conf file that would be ignored')
        ...
        # cosanta.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "cosanta.conf").write_text(f"datadir={node.datadir}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "cosanta.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/cosanta.conf" from data directory "{default_datadir}" '
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "cosanta.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_cosanta.conf")
