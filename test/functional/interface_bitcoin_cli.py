...
# The block reward of coinbaseoutput.nValue (500) COSA/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 500 COSA/block.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `cosanta-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and cosanta-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and cosanta-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (COSA/kB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (COSA/kB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (500 COSA) to the default wallet.
