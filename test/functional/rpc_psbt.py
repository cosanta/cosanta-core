...
class PSBTTest(BitcoinTestFramework):
    ...
    def test_input_confs_control(self):
        ...
        self.log.info("Fail to broadcast a new PSBT with maxconf 0 to verify it actually chose unconfirmed outputs")
        ...
        # The remaining unconfirmed outputs only cover the target together with an output of 'final_tx1',
        # and there is no RBF in Dash, so the transaction conflicts with the mempool
        invalid_inputs = self.nodes[0].decodepsbt(psbt_invalid)['tx']['vin']
        ...
        self.log.info("Craft a new PSBT adding inputs with highest confs possible")
    ...
    def run_test(self):
        # Create and fund a raw tx for sending 10 COSA
        ...
        self.log.info("Test walletcreatefundedpsbt fee rate of 10000 unit/B and 0.1 COSA/kvB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        res1 = self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"fee_rate": 10000, "add_inputs": True})
        assert_approx(res1["fee"], 0.04, 0.005)
        res2 = self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"feeRate": "0.1", "add_inputs": True})
        assert_approx(res2["fee"], 0.04, 0.005)

        self.log.info("Test min fee rate checks with walletcreatefundedpsbt are bypassed, e.g. a fee_rate under 1 unit/B is allowed")
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (unit/B) and feeRate (COSA/kB)",
