...
BASE_SCRIPTS = [
    ...
    'interface_zmq_cosanta.py --legacy-wallet',
    'interface_zmq.py',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, attempts=1, enable_coverage=False, args=None, combined_logs_len=0,failfast=False, use_term_control):
    ...
    # Warn if cosantad is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "cosantad"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a cosantad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
