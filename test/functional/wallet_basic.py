...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Sendmany with explicit fee (COSA/kB)
        ...
        # Sendmany with explicit fee (unit/B)
        # Throw if no conf_target provided
        assert_raises_rpc_error(-8, "Selected estimate_mode requires a fee rate",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            estimate_mode='unit/b')
        # Throw if negative feerate
        assert_raises_rpc_error(-3, "Amount out of range",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            ...
            estimate_mode='unit/b')
        fee_unit_per_b = 2
        fee_per_kb = fee_unit_per_b / 100000.0
        explicit_fee_per_byte = Decimal(fee_per_kb) / 1000
        txid = self.nodes[2].sendmany(
            amounts={ address: 10 },
            conf_target=fee_unit_per_b,
            estimate_mode='unit/b',
        )
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        if not self.options.descriptors:
            ...
            # send with explicit COSA/kB fee
            self.log.info("test explicit fee (sendtoaddress as COSA/kB)")
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
            ...
            # send with explicit unit/b fee
            self.sync_all(self.nodes[0:3])
            self.log.info("test explicit fee (sendtoaddress as unit/b)")
            self.nodes[0].generate(1)
            prebalance = self.nodes[2].getbalance()
            assert prebalance > 2
            address = self.nodes[1].getnewaddress()
