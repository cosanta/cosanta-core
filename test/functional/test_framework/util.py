...
def assert_fee_amount(fee, tx_size, feerate_DASH_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s COSA too low! (Should be %s COSA)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_DASH_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s COSA too high! (Should be %s COSA)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "cosanta.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "CosantaCore"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/CosantaCore"
        else:
            datadir = temp_dir / ".cosanta"
    return env, datadir


def append_config(datadir, options):
    with open(os.path.join(datadir, "cosanta.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "cosanta.conf")):
        with open(os.path.join(datadir, "cosanta.conf"), 'r', encoding='utf8') as f:
            for line in f:
