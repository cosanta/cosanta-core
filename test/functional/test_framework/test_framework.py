...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave cosantads and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop cosantads after the test execution")
        ...
        parser.add_argument("--cosantad-arg", "--dashd-arg", dest="cosantad_extra_args", default=[], action="append",
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "cosantad": ("bitcoind", "COSANTAD"),
            "cosanta-cli": ("bitcoincli", "COSANTACLI"),
            "cosanta-util": ("bitcoinutil", "COSANTAUTIL"),
            "cosanta-wallet": ("bitcoinwallet", "COSANTAWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: cosantads were not stopped and may still be running")
    ...
    def setup_nodes(self):
        ...
        # resend timer) are scheduled relative to it rather than to the cached chain.
        if not self.setup_clean_chain and not self.disable_mocktime:
    ...
    def dynamically_initialize_datadir(self, mnidx):
        ...
        write_config(os.path.join(new_data_dir, "cosanta.conf"),
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as cosantad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in cosanta.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if cosantad has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("cosantad has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoin_util(self):
        """Skip the running test if cosanta-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("cosanta-util has not been compiled")
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether cosanta-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether cosanta-util was compiled."""
