...
# Copyright (c) 2021-2026 The Cosanta Core developers
...
def exec_insert_header(filename, style):
    ...
    if int(end_year) < HOLDER_START_YEAR:
        sys.exit('*** %s has no changes from %s onwards; no Cosanta copyright inserted'
                 % (filename, HOLDER_START_YEAR))
    start_year = str(max(int(start_year), HOLDER_START_YEAR))
...
The start year is never earlier than 2021, when Cosanta development began.
If the file's history ends before 2021, no Cosanta copyright is inserted.
