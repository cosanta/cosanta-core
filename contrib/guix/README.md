# Bootstrappable Cosanta Core Builds

This directory contains the files necessary to perform bootstrappable Cosanta
Core builds.
...
To build Cosanta Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Cosanta Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/cosanta-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/cosanta/guix.sigs).
